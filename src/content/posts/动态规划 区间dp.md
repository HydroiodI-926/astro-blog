...
以上回文串解释了回文串的构成。当回文串的长度为奇数时，长度为1肯定是回文的，长度n大于1时，中心点为$n/2$（向下取整，下标从0开始）左右分布对应字符串对应相等如aba；当回文串为偶数时，长度为2时，左边起点定义为l，右边起点定义为r，l+1=r，长度大于2时，左边起点定义为l，右边起点定义为r，l+i=r-i$(0<=i<=n/2-1)$如abba
### 变成回文串的最小开销
- bca ->bcacb / acbca        2
- bcab -> bcacb / bacab    1
...
```cpp
void dfs(string s,int l,int r){ // 基于以上分析写的暴力
	if(l==r){  // a
		return 0;
	}
	if(l+1==r){ // aa
